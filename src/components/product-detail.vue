  ...
  <section class="main">
    <section class="news-nav-bg">
      <ol class="breadcrumb news-nav">
        ...
        <li><a href="product.html">产品大厅</a></li>
      </ol>
    </section>
    ...
    <section class="news-detail">
      ...
      <h4><a :href="item.po_link" target="_blank">查看详情</a></h4>
      <hr />
      <i class="news-date">
        <span class="ns-year">{{getYear(item.po_exeg)}}</span>
        <span class="ns-line"></span>
        <span class="ns-day">{{getDay(item.po_exeg)}}</span>
      </i>
      <div class="ns_content" v-html="item.po_content">
      </div>
      <div class="clear"></div>
    </section>
...
<script>
  ...
  export default {
    ...
    methods: {
      load() {
        var that = this
        var url = config.HttpUrl + '/handlers/product.ashx?method=get'
...
<style scoped>
  ...
  .news-detail { width: 90%; margin: 0 auto; }
    ...
    .news-detail h4 { text-align: center; font-size: 90%; }
      .news-detail h4 a { }
